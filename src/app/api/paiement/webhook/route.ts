...
export async function POST(request: Request) {
  ...
  try {
    ...
    const commande = await prisma.order.findUnique({
      where: { numero },
      include: { payment: true, user: true, address: true, items: { include: { productVariant: { include: { product: true } } } } },
    });
    ...
    const lignesEmail = commande.items.map((l) => ({ nom: l.productVariant.product.nom, quantite: l.quantite, prixUnitaire: l.prixUnitaire }));
