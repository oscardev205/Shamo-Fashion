...
// Fichier complet : le panier référence maintenant une variante (taille/couleur/prix)
// et non plus directement un produit à prix unique. Plus de notion de stock
// (Grace Débordée ne gère pas le stock), donc plus de plafond de quantité.
"use client";
...
type CartContextType = {
  ...
  retirerDuPanier: (variantId: string) => void;
  viderPanier: () => void;
...
const STORAGE_KEY = "grace-debordee-panier";
const STORAGE_KEY_PROMO = "grace-debordee-promo";
const STORAGE_KEY_POINTS = "grace-debordee-points-utilises";
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  function ajouterAuPanier(item: Omit<CartItem, "quantite">, quantite = 1) {
    setItems((prev) => {
      const existant = prev.find((i) => i.variantId === item.variantId);
      if (existant) {
        return prev.map((i) => (i.variantId === item.variantId ? { ...i, quantite: i.quantite + quantite } : i));
      }
      return [...prev, { ...item, quantite }];
    });
  }

  function modifierQuantite(variantId: string, quantite: number) {
    setItems((prev) =>
      quantite <= 0 ? prev.filter((i) => i.variantId !== variantId) : prev.map((i) => (i.variantId === variantId ? { ...i, quantite } : i))
    );
  }

  function retirerDuPanier(variantId: string) {
