...
import { formatPrix, libelleVarianteCommande } from "@/lib/format";
...
export default async function DetailLivraisonPage({
  ...
  const commande = await prisma.order.findUnique({
    where: { numero },
    include: { items: { include: { productVariant: { include: { product: true } } } }, address: true, user: true },
  });
      ...
      <div className="mb-4 divide-y divide-sable rounded-xl border border-sable">
        {commande.items.map((ligne) => (
          <div key={ligne.id} className="flex justify-between p-3 text-sm text-encre">
            <span>{libelleVarianteCommande(ligne.productVariant.product.nom, ligne.productVariant.taille, ligne.productVariant.couleur)} × {ligne.quantite}</span>
