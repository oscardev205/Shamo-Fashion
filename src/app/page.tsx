...
// gamme) — hero, valeurs et visuel remplacés, le reste de la structure
// (comment ça marche, produits phares, témoignages) est conservé.

import Link from "next/link";
import { FaGem, FaAward, FaCrown, FaShirt, FaUserTie, FaBasketShopping, FaLock, FaTruckFast, FaStar } from "react-icons/fa6";
    ...
    <main className="mx-auto max-w-6xl px-4">
      ...
      <section className="mt-6 grid gap-8 md:mt-8 md:grid-cols-2 md:items-center md:gap-12">
        <FadeIn>
          <span className="inline-block rounded-full bg-vivrebio-rouge/10 px-3 py-1 text-xs font-medium text-vivrebio-rouge">
            Costumes · Chemises · Chaussures
          </span>

          <h1 className="mt-5 text-2xl font-bold leading-tight text-encre sm:text-3xl md:text-4xl">
            Votre élégance,<br />notre priorité.
          </h1>
          <TraitFeuille className="mt-3" />

          <p className="mt-4 max-w-md text-sm text-encre/60">
            Costumes, chemises, chaussures, montres et accessoires haut de gamme pour
            homme — sélectionnés avec exigence, pour une allure soignée en toute
            occasion. Chaque article de notre catalogue est choisi pour sa coupe, sa
            qualité et sa finition.
          </p>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <Link href="/a-propos">
              <Button variant="outline">Notre histoire</Button>
            </Link>
          </div>
        </FadeIn>

        {/* Visuel hero : composition d'icônes mode (costume, chemise, accessoires),
            en écho au thème bleu marine/or de la marque. */}
        <FadeIn delai={150} className="relative aspect-[4/3] overflow-hidden rounded-3xl bg-vert-pale">
          <FaGem className="absolute left-[8%] top-[12%] rotate-[-15deg] text-3xl text-vivrebio-vert/25 sm:text-4xl" />
          <FaCrown className="absolute right-[12%] top-[18%] rotate-[12deg] text-2xl text-vivrebio-vert-clair/40 sm:text-3xl" />
          <div className="absolute inset-0 flex items-center justify-center">
            <FaUserTie className="text-6xl text-vivrebio-vert/25 sm:text-7xl md:text-8xl" />
          </div>
          <FaShirt className="absolute bottom-[15%] left-[16%] rotate-[8deg] text-2xl text-vivrebio-vert-clair/35 sm:text-3xl" />
          <FaAward className="absolute bottom-[10%] right-[10%] rotate-[-10deg] text-3xl text-vivrebio-vert/20 sm:text-4xl" />
          <div className="absolute right-[6%] top-[8%] h-16 w-16 rounded-full border-2 border-dashed border-vivrebio-vert/20" />
        </FadeIn>
      </section>
      ...
      <section className="mt-14 md:mt-20">
        <FadeIn>
          ...
          <p className="mt-3 max-w-xl text-sm text-encre/60">
            Commander vos articles préférés n&apos;a jamais été aussi simple — que vous
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-3">
          {[
            { icon: FaBasketShopping, titre: "Choisissez vos articles", texte: "Parcourez notre catalogue et ajoutez vos coups de cœur au panier." },
      ...
      <section className="mt-14 md:mt-20">
        <FadeIn>
          ...
          <h2 className="mt-1 text-lg font-bold text-encre sm:text-xl">Pourquoi Shamo Fashion</h2>
          <TraitFeuille className="mt-2" />
        </FadeIn>
      ...
      <section className="mt-14 md:mt-20">
        <FadeIn className="flex flex-wrap items-end justify-between gap-2">
          <div>
            ...
            <h2 className="mt-1 text-lg font-bold text-encre sm:text-xl">Nos articles phares</h2>
            <TraitFeuille className="mt-2" />
          </div>
