...
export function ProductImage({ nom, imageUrl, className }: Props) {
  ...
  if (!imageUrl || erreur) {
    return (
      <div className={`flex h-full w-full items-center justify-center rounded-lg bg-vert-pale ${className ?? ""}`}>
  ...
  return (
    <Image
      ...
      className={`object-cover transition-transform duration-500 ease-out group-hover:scale-105 motion-reduce:transition-none motion-reduce:transform-none ${className ?? ""}`}
