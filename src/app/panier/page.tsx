...
// Fichier complet : les lignes du panier sont désormais indexées par variante
// (variantId) et affichent la taille/couleur choisie. Plus de plafond de stock.
"use client";
    ...
    <main className="mx-auto max-w-4xl px-4 py-8">
      ...
      <div className="flex flex-col gap-8 md:flex-row">
        <div className="flex-1 divide-y divide-sable">
          {items.map((item) => (
            <div key={item.variantId} className="flex flex-col gap-3 py-3 sm:flex-row sm:items-center sm:gap-4">
              <div className="flex-1">
                <p className="text-sm font-medium text-encre">{item.nom}</p>
                {(item.taille || item.couleur) && (
                  <p className="text-xs text-encre/50">{[item.taille, item.couleur].filter(Boolean).join(" · ")}</p>
                )}
              ...
              <div className="flex items-center justify-between gap-3 sm:justify-normal">
                <div className="flex items-center rounded-lg border border-sable">
                  <button className="px-2.5 py-1 text-sm text-encre" onClick={() => modifierQuantite(item.variantId, item.quantite - 1)}>−</button>
                ...
                <button onClick={() => retirerDuPanier(item.variantId)} aria-label="Retirer l'article"><Trash2 size={16} className="text-encre/30 hover:text-vivrebio-rouge" /></button>
              </div>
            </div>
          ))}
        </div>
