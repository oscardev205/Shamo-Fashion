...
// propriétaire de la commande OU à un admin.
// Uniquement pour les commandes réellement payées.
...
export async function GET(
  ...
  const buffer = await renderToBuffer(
    FactureDocument({
      ...
      adresseLigne1: commande.address
        ? `${commande.address.adresseDetail}, ${commande.address.quartier}`
        : undefined,

      adresseLigne2: commande.address
        ? commande.address.ville
        : undefined,

      lignes: commande.items.map((l) => ({
        nom: l.productVariant.product.nom,
        quantite: l.quantite,
